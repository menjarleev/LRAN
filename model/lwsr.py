...
class AttentionBlock(nn.Module):
    def __init__(self, num_channels, res_scale, kernel_size, attn_type='channel', reduction=16, padding_mode='zeros'):
        super(AttentionBlock, self).__init__()
        self.scale = res_scale
        body = []
        body += [nn.Conv2d(num_channels, num_channels, kernel_size=kernel_size,stride=1, padding=kernel_size // 2, padding_mode=padding_mode)]
        if attn_type == 'channel':
            body += [ChannelAttention(num_channels, reduction, padding_mode=padding_mode)]
        elif attn_type == 'spatial':
            body += [SpatialAttention(7, padding_mode=padding_mode)]
...
class Group(nn.Module):
    def __init__(self, num_rf_block, num_ex_block, num_channels, reduction, actv=nn.ReLU, padding_mode='reflect', res_scale=1.0):
        super(Group, self).__init__()
        rf = []
        for i in range(num_rf_block):
            rf += [ARB(num_channels, res_scale, kernel_size=3, attn_type='spatial', reduction=reduction, actv=actv, padding_mode=padding_mode)]
        rf += [nn.Conv2d(num_channels, num_channels, 1, 1, 0, 1, padding_mode=padding_mode)]
        self.rf = nn.Sequential(*rf)
        ex = []
        for i in range(num_ex_block):
            ex += [ARB(num_channels, res_scale, kernel_size=1, attn_type='channel', reduction=reduction, actv=actv, padding_mode=padding_mode)]
        ex += [nn.Conv2d(num_channels, num_channels, 1, 1, 0, 1, padding_mode=padding_mode)]
        self.ex = nn.Sequential(*ex)
        self.fusion = nn.Conv2d(num_channels, num_channels, 1, 1, 0, 1, padding_mode=padding_mode)
...
class Net(nn.Module):
    def __init__(self, opt):
        ...
        body = []
        assert len(opt.num_blocks) == opt.num_groups
        for i in range(opt.num_groups):
            body += [Group(opt.num_blocks[i][0], opt.num_blocks[i][1], opt.num_channels, opt.reduction, actv,

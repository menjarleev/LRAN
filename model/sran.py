...
class Group(nn.Module):
    def __init__(self, num_rf_block, num_ex_block, num_channels, reduction, actv=nn.ReLU, padding_mode='reflect', res_scale=1.0):
        ...
        self.fusion1 = nn.Conv2d(num_channels, num_channels, kernel_size=3, stride=1, padding=1, padding_mode=padding_mode)
        self.fusion2 = nn.Conv2d(num_channels, num_channels, kernel_size=3, stride=1, padding=1, padding_mode=padding_mode)
...
class Net(nn.Module):
    def __init__(self, opt):
        ...
        for i in range(opt.num_groups):
            body += [Group(opt.num_blocks[i][0], opt.num_blocks[i][1],
                           opt.num_channels, opt.reduction, actv,

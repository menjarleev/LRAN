...
def get_PRAB_group(n_channel, n_block, PRAB_dict, res_scale=1, kernel_size=7, reduction=16,
        ...
        class Module(nn.Module):
            def __init__(self, num_channel, num_block, class_dict):
                ...
                self.agg = CSAB(num_channel * (self.group_size - 1), num_channel, reduction=reduction, actv=actv,
                                kernel_size=kernel_size, padding_mode=padding_mode)
                self.fusion = CSAB(num_channel, num_channel, reduction=reduction,
                                   actv=actv, kernel_size=kernel_size, padding_mode=padding_mode)

...
class SpatialAttention(nn.Module):
    def __init__(self, kernel_size=7, actv=nn.ReLU, padding_mode='reflect'):
        super(SpatialAttention, self).__init__()
        assert kernel_size in (3, 7), 'kernel size must be 3 or 7'
        padding = 3 if kernel_size == 7 else 1
        self.fc1 = nn.Conv2d(2, 1, kernel_size, padding=padding, padding_mode=padding_mode, bias=False)
        self.actv = actv()
        self.fc2 = nn.Conv2d(1, 1, kernel_size, padding=padding, padding_mode=padding_mode, bias=False)
        self.sigmoid = nn.Sigmoid()
    ...
    def forward(self, x):
        ...
        x = self.fc2(self.actv(self.fc1(x)))
        x = self.sigmoid(x)
        return x
...
class RCSAB(nn.Module):
    def __init__(self, num_channels, res_scale=1, reduction=16, actv=nn.ReLU, kernel_size=3, padding_mode='zeros', skip_conn=True):
